...
table_colors = session.sql("select color_or_style from catalog_for_website order by color_or_style")
pd_colors = table_colors.to_pandas()
    
st.write(pd_colors)

# Oyt the list of colors into a drop list selector 
option = st.selectbox('Pick a sweatsuit color or style:', pd_colors,key="color_selector")
